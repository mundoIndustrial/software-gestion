...
echo [4/6] Actualizando .env.development...
REM Actualizar variables críticas del .env
powershell -Command "$content = Get-Content '.env.development'; $content = $content -replace 'VITE_HMR_HOST=.*', 'VITE_HMR_HOST=%IP%'; $content | Set-Content '.env.development'; Write-Host '✓ VITE_HMR_HOST=%IP%'"
echo.
