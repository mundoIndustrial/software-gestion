...
echo "=== ANÁLISIS COMPLETO DE FILTRO DESCRIPCION_PRENDAS ===\n\n";
...
echo "Pedidos devueltos por filtro fuzzy: $totalMatched\n";
...
foreach ($napolesDropdown as $dropdownValue) {
    ...
    $words = array_filter(
        explode(' ', $normalized),
        function($word) {
            return strlen($word) >= 3 || preg_match('/[a-záéíóúñü]/i', $word);
        }
    );
    ...
    foreach ($words as $word) {
        $query->whereRaw(
            "LOWER(REPLACE(REPLACE(REPLACE(descripcion, '\r', ''), '\n', ''), ',', '')) LIKE ?",
            ['%' . strtolower($word) . '%']
        );
    }
