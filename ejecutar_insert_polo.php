<?php
/**
 * Script para ejecutar el SQL generado y reemplazar todos los datos
 */
...
$sqlFiles = glob(__DIR__ . '/insert_polo_desde_excel_*.sql');
if (empty($sqlFiles)) {
...
$sqlFile = $sqlFiles[0];
echo "=== EJECUTANDO SQL - REGISTRO_PISO_POLO ===\n\n";
...
try {
    ...
    $pdo->exec("TRUNCATE TABLE registro_piso_polo");
    ...
    $insertStart = stripos($sqlContent, 'INSERT INTO registro_piso_polo');
    ...
    try {
        ...
        $totalInsertados = $pdo->query("SELECT COUNT(*) FROM registro_piso_polo")->fetchColumn();
    ...
    // Verificar HORA 08 específicamente
    echo "REGISTROS DE HORA 08:\n";
    $stmt = $pdo->query("
        SELECT 
            COUNT(*) as total_registros,
            SUM(cantidad) as suma_cantidad
    ...
    echo "  Total: {$result['total_registros']} registros | Cantidad: {$result['suma_cantidad']}\n\n";
