 ...
 * - Convertir a WebP
 * - Guardar en carpeta especifica del pedido
 * - Retornar rutas para BD
 */
class TelaFotoService
{
    private const WEBP_QUALITY = 80;

    /**
     * Procesar y guardar foto de tela
     * 
     * @param UploadedFile $archivo
     * @param int|null $pedidoId - Opcional para organizarlas por pedido
     * @param bool $soloWebp - Si true, solo guarda WebP (modo edicion). Si false, guarda original + WebP
    ...
    public function procesarFoto(UploadedFile $archivo, ?int $pedidoId = null, bool $soloWebp = true): array
    {
        ...
        if ($soloWebp) {
            ...
            return [
                'ruta_original' => $rutaWebp,  // Retornar WebP como original tambien
                'ruta_webp' => $rutaWebp,
            ];
        }
    ...
    /**
     * Guardar imagen original con nombre ano
     * 
     * @param UploadedFile $archivo
    ...
    private function guardarOriginal(UploadedFile $archivo, ?int $pedidoId = null): string
    {
        ...
        // Guardar en carpeta especifica si pedidoId existe
        if ($pedidoId) {
    ...
    private function convertirAWebp(string $rutaOriginal): string
    {
        try {
            ...
            \Log::warning('[TelaFotoService] Error convertiendo a WebP', [
                'ruta_original' => $rutaOriginal,
                'error' => $e->getMessage(),
            ]);
            
            // Si falla conversion, retornar original
            return $rutaOriginal;
        }
    }
    ...
    /**
     * Generar nombre ano para archivo
     * 
     * @param UploadedFile $archivo
    ...
    private function generarNombreUnico(UploadedFile $archivo): string
    {
        ...
        return "tela_{$timestamp}_{$hash}.{$extension}";
    }
}
