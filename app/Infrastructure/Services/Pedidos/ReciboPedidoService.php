 ...
 */
class ReciboPedidoService implements ReciboPedidoServiceContract
{
    /**
    ...
    public function obtenerDatosRecibos(int $pedidoId, bool $filtrarProcesosPendientes = false): array
    {
        ...
        $datos = [
            ...
            'fecha' => $pedido->created_at ? $pedido->created_at->format('d/m/Y h:i A') : date('d/m/Y h:i A'),
            'fecha_creacion' => $pedido->created_at ? $pedido->created_at->format('d/m/Y h:i A') : date('d/m/Y h:i A'),
    ...
    private function procesarProcesosParaRecibo($prenda, ?int $colorTelaId): array
    {
        ...
        foreach ($prenda->procesos as $proc) {
            ...
            if (in_array($modoTallas, ['general', 'especifico'])) {
                ...
                \Log::info('[RECIBO-SERVICE] Modo generico (sin detalles por talla)', [
                    'procesoId' => $proc->id,
                    'modo' => $modoTallas
                ]);
            }
    ...
    public function call(string $method, array $arguments = []): mixed
    {
        if (!method_exists($this, $method)) {
            throw new \BadMethodCallException("Method {ReciboPedidoService}::$method does not exist");
        }
