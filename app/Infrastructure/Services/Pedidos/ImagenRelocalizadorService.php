 ...
 * 
 * Responsabilidad: Mover imagenes de temp/{uuid}/ a pedidos/{pedido_id}/{tipo}/
 * Usado cuando se crea un pedido para consolidar las imagenes subidas
 */
class ImagenRelocalizadorService
{
    /**
     * Mover imagenes de temp a estructura pedidos/{pedido_id}/{tipo}/
     * 
     * @param int $pedidoId
     * @param array $rutasTemp Rutas en formato: ['prendas/temp/uuid/file.webp', 'telas/temp/uuid/file.webp']
     * @param string|null $tipoEspecifico Fuerza un tipo especifico ('prendas', 'telas', 'procesos')
     ...
     * @param string|null $tipoEspecifico Fuerza un tipo especifico
    ...
    private function moverImagen(int $pedidoId, string $rutaTemp, ?string $tipoEspecifico = null): ?string
    {
        ...
        try {
            ...
            Log::error('[ImagenRelocalizadorService] Error durante recolizacion', [
                'pedido_id' => $pedidoId,
                'ruta_temp' => $rutaTemp,
                'ruta_final' => $rutaFinal,
                'error' => $e->getMessage(),
    ...
    /**
     * Limpiar carpeta temporal si queda vacua
     * Limpia recursivamente hasta temp/{uuid}/ si todo queda vacia
     * 
    ...
    private function limpiarCarpetaTempSiVacia(string $carpeta): void
    {
        try {
            ...
            // Si esta vacia (sin archivos ni subdirectorios), eliminarla
            if (empty($archivos) && empty($subdirectorios)) {
                ...
                // Limpiar carpeta padre si tambien queda vacia
