 ...
 * 
 * Mapea imagenes a sus entidades despues de crear en BD
 * 
 * FLUJO:
...
class MapeoImagenesService
{
    ...
    /**
     * Mapear y crear registros de imagenes despues de crear entidades en BD
     * 
    ...
    public function mapearYCrearFotos(
        ...
        // Resolver todas las imagenes (extraer, procesar, mapear)
    ...
    private function crearRegistrosPrendas(
        ...
        foreach ($dto->prendas as $prendaDTO) {
            ...
            foreach ($prendaDTO['imagenes'] as $imgIdx => $imagenDTO) {
                ...
                // Intentar con formdata_key primero (mas especificos), luego con uid
                $rutaFinal = null;
                if ($formDataKey && isset($mapeoUidARuta[$formDataKey])) {
                    ...
                    // Usar ruta existente (cotizacion o borrador guardado previo)
            ...
            foreach ($prendaDTO['telas'] as $telaIdx => $telaDTO) {
                ...
                foreach ($telaDTO['imagenes'] as $imgIdx => $imagenDTO) {
                    ...
                    // Intentar con formdata_key primero (mas especifico), luego con uid
                    $rutaFinal = null;
                    if ($formDataKey && isset($mapeoUidARuta[$formDataKey])) {
                        $rutaFinal = $mapeoUidARuta[$formDataKey];
            ...
            // DEBUG: Verificar que se guarda en datos_adicionales
            ...
            foreach ($prendaDTO['procesos'] as $procesoIdx => $procesoDTO) {
                ...
                if (!$procesoEnBD) {
                    // Si no encuentra por UID, intentar busqueda alternativa por tipo de proceso
                    // (Por si el UID no se guarda correctamente)
                    // Usar el NOMBRE DEL TIPO desde el DTO ($procesoDTO['tipo'])
                    Log::debug('[MapeoImagenesService] busqueda alternativa por tipo', [
        ...
        Log::info('[MapeoImagenesService] Mapeo de imagenes completado', [
            'pedido_id' => $pedidoId,
        ]);
    }
}
