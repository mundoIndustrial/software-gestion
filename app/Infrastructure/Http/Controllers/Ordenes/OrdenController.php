...
namespace App\Infrastructure\Http\Controllers\Ordenes;
...
/**
 * OrdenController - API DDD Layer
 * 
 * Responsabilidad única: Exponer Application Services de DDD vía HTTP
 * Cumple: SRP, DIP, OCP
 * 
 * Endpoints:
 * - GET    /api/v1/ordenes              (index)
 * - GET    /api/v1/ordenes/{numero}     (show)
 * - POST   /api/v1/ordenes              (store)
 * - PATCH  /api/v1/ordenes/{numero}/aprobar        (aprobar)
 * - PATCH  /api/v1/ordenes/{numero}/iniciar-produccion (iniciarProduccion)
 * - PATCH  /api/v1/ordenes/{numero}/completar (completar)
 * - DELETE /api/v1/ordenes/{numero}     (destroy)
 * - GET    /api/v1/ordenes/cliente/{cliente} (porCliente)
 * - GET    /api/v1/ordenes/estado/{estado} (porEstado)
 * 
 * Arquitectura: Repository → DomainService → Controller
 */
class OrdenController extends Controller
{
    public function __construct(
        private readonly CrearOrdenService $crearOrdenService,
        private readonly ActualizarEstadoOrdenService $actualizarEstadoService,
        private readonly CancelarOrdenService $cancelarOrdenService,
        private readonly ObtenerOrdenService $obtenerOrdenService
    ) {}
    ...
    public function index()
    {
        try {
            ...
            \Log::error('Error obteniendo ordenes', ['error' => $e->getMessage()]);
    ...
    public function show(int $numero)
    {
        try {
            ...
            \Log::error('Error obteniendo orden', ['numero' => $numero, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function porCliente(string $cliente)
    {
        try {
            ...
            \Log::error('Error obteniendo ordenes por cliente', ['cliente' => $cliente, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function porEstado(string $estado)
    {
        try {
            ...
            \Log::error('Error obteniendo ordenes por estado', ['estado' => $estado, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            \Log::error('Error creando orden', ['error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function aprobar(int $numero)
    {
        try {
            ...
            \Log::error('Error aprobando orden', ['numero' => $numero, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function iniciarProduccion(int $numero)
    {
        try {
            ...
            \Log::error('Error iniciando producción', ['numero' => $numero, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function completar(int $numero)
    {
        try {
            ...
            \Log::error('Error completando orden', ['numero' => $numero, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function destroy(int $numero)
    {
        try {
            ...
            \Log::error('Error cancelando orden', ['numero' => $numero, 'error' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
