...
class EntregaController extends Controller
{
    ...
    public function garments(Request $request)
    {
        ...
        try {
            if ($tipo === 'pedido') {
                // Obtener el pedido de produccion por numero de pedido
                ...
                return response()->json(['error' => 'Tipo invalido'], 400);
            }
    ...
    public function sizes(Request $request)
    {
        ...
        try {
            if ($tipo === 'pedido') {
                ...
                // Obtener la prenda especifica desde prendas_pedido
                ...
                // Extraer tallas desde la relacion
        ...
        try {
            ...
            if ($subtipo === 'costura') {
                foreach ($entregas as $entrega) {
                    ...
                    if ($tipo === 'pedido') {
                        // Obtener el pedido de produccion
                        ...
                        if (!$entregaPrendaPedido) {
                            ...
                            if ($totalNuevo > $cantidadOriginal) {
                                return response()->json([
                                    'success' => false,
                                    'message' => "No se puede entregar mas de {$cantidadOriginal} unidades. Ya se han entregado {$entregaPrendaPedido->total_producido_por_talla}. Intenta entregar maximo " . ($cantidadOriginal - $entregaPrendaPedido->total_producido_por_talla) . " unidades mas."
                                ], 422);
                            }
                        ...
                        // Tambien guardar en entrega_pedido_costura (tabla legacy)
                        try {
                        ...
                        if (!$registro) {
                            return response()->json([
                                'success' => false, 
                                'message' => "Registro de produccion no encontrado para Pedido {$entrega['pedido']}, Prenda {$entrega['prenda']}, Talla {$entrega['talla']}"
                            ], 404);
                        }
                    ...
                    try {
                        ...
                        \Log::warning('Broadcast error (no critico)', [
