...
class TablaOriginalBodegaObserver
{
    /**
     * Handle the TablaOriginalBodega "updated" event.
     */
    public function updated(TablaOriginalBodega $orden)
    {
     ...
     */
    private function sincronizarPrendasConHijos(TablaOriginalBodega $orden)
    {
        try {
            ...
            foreach ($prendasNuevas as $index => $prendaNueva) {
                ...
                if ($prendaAntigua['nombre'] !== $prendaNueva['nombre']) {
                    $actualizados = DB::table('registros_por_orden_bodega')
                        ->where('pedido', $orden->pedido)
                        ->where('prenda', $prendaAntigua['nombre'])
                        ->update(['prenda' => $prendaNueva['nombre']]);

                    Log::info("Prenda actualizada en registros hijos (Bodega)", [
                        'pedido' => $orden->pedido,
                ...
                if ($prendaAntigua['descripcion'] !== $prendaNueva['descripcion']) {
                    ...
                    Log::info("Descripción actualizada en registros hijos (Bodega)", [
                        'pedido' => $orden->pedido,
            ...
            Log::error("Error sincronizando prendas con hijos (Bodega)", [
                'pedido' => $orden->pedido,
                'error' => $e->getMessage(),
     ...
     */
    private function sincronizarClienteConHijos(TablaOriginalBodega $orden)
    {
        try {
            $actualizados = DB::table('registros_por_orden_bodega')
                ->where('pedido', $orden->pedido)
            ...
            Log::info("Cliente actualizado en registros hijos (Bodega)", [
                'pedido' => $orden->pedido,
            ...
            Log::error("Error sincronizando cliente con hijos (Bodega)", [
                'pedido' => $orden->pedido,
                'error' => $e->getMessage()
            ]);
        }
