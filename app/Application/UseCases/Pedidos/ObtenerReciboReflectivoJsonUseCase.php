...
/**
 * UseCase: ObtenerReciboReflectivoJsonUseCase
 * 
 * Responsabilidad: Obtener datos de un recibo REFLECTIVO específico como JSON
 * Entrada: ID del recibo
 * Salida: JSON con datos del recibo enriquecidos
 * 
 * Endpoint: GET /api/recibos-reflectivo/{reciboId}
 */
class ObtenerReciboReflectivoJsonUseCase
{
    ...
    public function execute(int $reciboId): array
    {
        try {
            ...
            $recibo = $this->recibosRepository->obtenerReciboReflectivo($reciboId);
            ...
            if (empty($nombrePrenda) || $nombrePrenda === 'Sin prendas') {
                try {
                    ...
                    Log::warning('[ObtenerReciboReflectivoJsonUseCase] Error obteniendo nombre prenda', [
                        'recibo_id' => $reciboId,
                        'error' => $e->getMessage(),
                    ]);
                }
            ...
            Log::error('[ObtenerReciboReflectivoJsonUseCase] Error', [
                'recibo_id' => $reciboId,
                'error' => $e->getMessage(),
            ]);
    ...
    private function cargarFestivos(): array
    {
        try {
            ...
            Log::warning('[ObtenerReciboReflectivoJsonUseCase] No se pudieron cargar festivales', [
                'error' => $e->getMessage(),
            ]);
            return [];
        }
