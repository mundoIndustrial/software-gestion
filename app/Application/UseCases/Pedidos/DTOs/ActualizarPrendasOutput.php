<?php

namespace App\Application\UseCases\Pedidos\DTOs;

/**
 * DTO para salida de ActualizarPrendasUseCase
 * 
 * Responsabilidad: Encapsular resultado de actualizar prendas
 * Patrón: Transfer Object
 */
class ActualizarPrendasOutput
{
    public function __construct(
        public int $numero_pedido,
        public string $mensaje,
        public int $total_prendas_actualizado,
        public ?array $orden_actualizada = null,
        public ?array $metadata = null,
    ) {}

    /**
     * Convertir a array para response JSON
     */
    public function toArray(): array
    {
        return [
            'numero_pedido' => $this->numero_pedido,
            'mensaje' => $this->mensaje,
            'total_prendas' => $this->total_prendas_actualizado,
            'orden_actualizada' => $this->orden_actualizada,
            'metadata' => $this->metadata,
        ];
    }

    /**
     * Convertir a response JSON
     */
    public function toResponse(): array
    {
        return array_merge(
            $this->toArray(),
            ['success' => true]
        );
    }
}
