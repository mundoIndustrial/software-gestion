...
class ObtenerPedidosAprobadosUseCase
{
    ...
    {
        try {
            Log::debug('ObtenerPedidosAprobadosUseCase ejecutado', $filtros);

            $resultado = $this->repository->obtenerPedidosAprobados(
                $filtros['page'] ?? 1,
                $filtros['per_page'] ?? 15,
                $filtros['search'] ?? '',
                $filtros['cliente'] ?? '',
            ...
            Log::error('Error en ObtenerPedidosAprobadosUseCase: ' . $e->getMessage());
            return [
                'success' => false,
