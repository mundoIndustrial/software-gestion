...
class UpdateOrderWriteService
{
    private Request $request;

    public function update(UpdateOrderRequest $dtoRequest, Request $request): ?PedidoProduccion
    {
        $this->request = $request;

        $orden = $this->findOrderForUpdate($dtoRequest->getOrderId());
    ...
    protected function findOrderForUpdate(int $orderId): PedidoProduccion
    {
        return PedidoProduccion::with('prendas')->findOrFail($orderId);
    }
    ...
    {
        $changedFields = [];
        if (!empty($this->request->input('cliente'))) {
            $changedFields[] = 'cliente';
        }
        if (!empty($this->request->input('forma_de_pago'))) {
            $changedFields[] = 'forma_de_pago';
        }
        if (!empty($this->request->input('novedades'))) {
            $changedFields[] = 'novedades';
        }
        if (!empty($this->request->input('dia_de_entrega'))) {
            $changedFields[] = 'dia_de_entrega';
        }
        if (!empty($this->request->input('fecha_estimada_de_entrega'))) {
