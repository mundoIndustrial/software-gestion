...
class GetPendingReflectiveReceiptsUseCase
{
    ...
    {
        try {
            $recibosReflectivo = collect(
                $this->receiptRepository->findPendingReflectiveReceipts(
                    $this->buildFilters($request)
                )
            );

            Log::info('Recibos REFLECTIVO recuperados', ['count' => $recibosReflectivo->count()]);

            $procesosConCantidad = $recibosReflectivo->map(function ($recibo) {
                return $this->formatReceipt($recibo);
            });

            return new GetPendingSewingReceiptsResponse($procesosConCantidad->toArray());
        } catch (\Exception $e) {
            Log::error('Error en GetPendingReflectiveReceipts: ' . $e->getMessage());
            throw $e;
        }
    }
