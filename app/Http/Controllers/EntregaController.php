...
class EntregaController extends Controller
{
    ...
    public function garments(Request $request)
    {
        ...
        try {
            ...
            // Buscar en registros_por_orden (pedido)
            $garmentsPedido = \App\Models\RegistrosPorOrden::where('pedido', $pedido)
                ->select('prenda')
                ->distinct()
                ->get()
                ->pluck('prenda');
            $garments = $garments->merge($garmentsPedido);

            // Buscar en registros_por_orden_bodega (bodega)
            $garmentsBodega = \App\Models\RegistrosPorOrdenBodega::where('pedido', $pedido)
                ->select('prenda')
                ->distinct()
                ->get()
                ->pluck('prenda');
            $garments = $garments->merge($garmentsBodega);

            // Eliminar duplicados
            $garments = $garments->unique();

            return response()->json($garments->values());
        } catch (\Exception $e) {
    ...
    public function sizes(Request $request)
    {
        ...
        // Buscar en registros_por_orden (pedido)
        $sizesPedido = \App\Models\RegistrosPorOrden::where('pedido', $pedido)
            ->where('prenda', $prenda)
            ->get();
        $sizes = $sizes->merge($sizesPedido);

        // Buscar en registros_por_orden_bodega (bodega)
        $sizesBodega = \App\Models\RegistrosPorOrdenBodega::where('pedido', $pedido)
            ->where('prenda', $prenda)
            ->get();
        $sizes = $sizes->merge($sizesBodega);
        ...
        try {
            ...
            if ($subtipo === 'costura') {
                foreach ($entregas as $entrega) {
                    ...
                    // Buscar en registros_por_orden primero
                    ...
                    if ($registro) {
                        ...
                        // Si no se encontró, buscar en registros_por_orden_bodega
                    ...
                    // Buscar en ambas tablas para actualizar correctamente
                    $updated = false;

                    // Intentar actualizar en registros_por_orden
                    $registroPedido = \App\Models\RegistrosPorOrden::where('pedido', $entrega['pedido'])
                        ->where('prenda', $entrega['prenda'])
                        ->where('talla', $entrega['talla'])
                        ->first();

                    if ($registroPedido) {
                        $registroPedido->decrement('total_pendiente_por_talla', $entrega['cantidad_entregada']);
                        $registroPedido->increment('total_producido_por_talla', $entrega['cantidad_entregada']);
                        $updated = true;
                    }

                    // Si no se encontró en pedido, intentar en bodega
                    if (!$updated) {
                        $registroBodega = \App\Models\RegistrosPorOrdenBodega::where('pedido', $entrega['pedido'])
                            ->where('prenda', $entrega['prenda'])
                            ->where('talla', $entrega['talla'])
                            ->first();

                        if ($registroBodega) {
                            $registroBodega->decrement('total_pendiente_por_talla', $entrega['cantidad_entregada']);
                            $registroBodega->increment('total_producido_por_talla', $entrega['cantidad_entregada']);
                        }
                    }
                }
                ...
                foreach ($entregas as $entrega) {
                    ...
                    // Get prendas internally from both tables
                    $prendas = collect();

                    $prendasPedido = \App\Models\RegistrosPorOrden::where('pedido', $entrega['pedido'])
                        ->select('prenda')
                        ->distinct()
                        ->get()
                        ->pluck('prenda');
                    $prendas = $prendas->merge($prendasPedido);

                    $prendasBodega = \App\Models\RegistrosPorOrdenBodega::where('pedido', $entrega['pedido'])
                        ->select('prenda')
                        ->distinct()
                        ->get()
                        ->pluck('prenda');
                    $prendas = $prendas->merge($prendasBodega);

                    $prendas = $prendas->unique()->toArray();
