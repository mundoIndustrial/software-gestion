...
class ImportarSoloProduccion extends Command
{
    protected $signature = 'importar:solo-produccion 
                            {archivo? : Ruta al archivo Excel de PRODUCCION}
                            {--limpiar : Eliminar registros existentes antes de importar}';
    
    protected $description = 'Importar solo registros de PRODUCCION desde archivo Excel';

    public function handle()
    {
        $archivo = $this->argument('archivo') ?: resource_path('CONTROL DE PISO PRODUCCION (respuestas) (1).xlsx');
        $limpiar = $this->option('limpiar');

        $this->info("╔════════════════════════════════════════════════════════════╗");
        $this->info("║       IMPORTACIÓN DE REGISTRO PISO PRODUCCION              ║");
        $this->info("╚════════════════════════════════════════════════════════════╝");
        $this->newLine();
        ...
        try {
            $procesados = $this->importarProduccion($archivo);
    ...
    private function limpiarTabla()
    {
        ...
        $count = DB::table('registro_piso_produccion')->count();
        DB::table('registro_piso_produccion')->truncate();
        $this->line("   ✓ registro_piso_produccion ({$count} registros eliminados)");
    ...
    private function importarProduccion($archivo)
    {
        $this->info("═══════════════════════════════════════════════════════════");
