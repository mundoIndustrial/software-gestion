...
function renderizarPrendasTipoPrendaSinCotizacion() {
    ...
    console.log('🎯 [RENDER] Gestor.prendas.length:', window.gestorPrendaSinCotizacion.prendas.length);
    console.log('🎯 [RENDER] Prendas eliminadas:', Array.from(window.gestorPrendaSinCotizacion.prendasEliminadas));
    console.log('🎯 [RENDER] Contenido prendas:', prendas);
    ...
    console.log('🎯 [RENDER] Actualizando innerHTML del container con', prendas.length, 'prendas');
    container.innerHTML = html;
    console.log('✅ [RENDER] innerHTML actualizado');
...
function renderizarBodegaPrendaTipo(prenda, index) {
    ...
    return `
        <div style="margin-top: 2rem; padding: 1rem; background: linear-gradient(135deg, #f0f7ff 0%, #e0efff 100%); border: 2px solid #1e40af; border-radius: 6px;">
            <label style="display: flex; align-items: center; gap: 0.75rem; font-weight: 600; color: #1e40af; margin-bottom: 0.75rem;">
                <i class="fas fa-location-dot" style="color: #1e40af; font-size: 1.1rem;"></i>
                <span>ORIGEN DE LA PRENDA *</span>
            </label>
            <select id="origen_prenda_${index}"
                    class="select-origen-prenda"
                    data-prenda-index="${index}"
                    style="width: 100%; padding: 0.75rem; border: 1px solid #1e40af; border-radius: 6px; font-size: 1rem; color: #1e40af; background-color: white; cursor: pointer;"
                    onchange="actualizarOrigenPrenda(${index}, this.value)">
                <option value="bodega" ${origen === 'bodega' ? 'selected' : ''}>📦 Bodega</option>
                <option value="confeccion" ${origen === 'confeccion' ? 'selected' : ''}>🏭 Confección</option>
            </select>
            <small style="display: block; margin-top: 0.5rem; color: #666;">
                ${origen === 'bodega' ? 'Esta prenda se saca de bodega existente' : 'Esta prenda se fabrica en confección'}
            </small>
        </div>
    `;
}
