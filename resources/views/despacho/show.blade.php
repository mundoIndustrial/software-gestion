...
function connectWebSocket() {
    if (!window.Echo || !window.EchoInstance) {
        console.error(' Echo no esta inicializado.');
    ...
    try {
        ...
        // Unirse al canal publico de despacho
        socket.channel('pedidos.general')
            .listen('.pedido.actualizado', (event) => {
                ...
                // Si el pedido cambio a "Entregado" y es el pedido actual, mostrar notificacion
...
// Funcion para mostrar notificacion local
function mostrarNotificacionPedidoEntregadoLocal(numeroPedido) {
    ...
    // Mostrar notificacion inmediatamente
...
function toggleHistorialEpp(btn, historialHomologaciones) {
    ...
    if (!Array.isArray(historialHomologaciones) || historialHomologaciones.length === 0) {
        console.warn(' [toggleHistorialEpp] Sin historial o historial vacio');
        // Si Swal no esta disponible, usar alert
        if (window.Swal) {
            ...
            <table class="w-full border-collapse text-sm">
                <thead style="position: sticky; top: 0; z-index: 10;">
                    <tr class="bg-blue-600 text-white shadow-md">
                        <th class="px-2 py-3 text-center font-bold w-36 whitespace-nowrap">Version</th>
    ...
    // Mostrar modal si Swal esta disponible, sino usar un alert mejorado
    if (window.Swal) {
        ...
        console.log('INFO: Swal no disponible, usando modal simple');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Usar el sistema waitForEcho para asegurar que Echo este disponible
...
<div class="min-h-screen bg-white">
    <div class="max-w-6xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex gap-3">
                    ...
                    <div class="relative z-[120]" id="imprimirMenuWrapper">
                        <button type="button"
                                id="btnImprimirMenu"
                                onclick="toggleMenuImpresion(event)"
                                class="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-medium rounded transition-colors inline-flex items-center gap-2">
                            Imprimir
                            <span class="text-xs">▾</span>
                        </button>
                        <div id="imprimirMenuOpciones"
                             class="hidden absolute right-0 top-full mt-2 w-52 bg-white border border-slate-300 rounded-lg shadow-2xl ring-1 ring-black/10 z-[9999] overflow-hidden">
                            <button type="button"
                                    onclick="imprimirTablaVacia('all')"
                                    class="block w-full text-left px-4 py-2.5 text-sm font-medium text-slate-800 hover:bg-slate-100 transition-colors">
                                Imprimir todo
                            </button>
                            <button type="button"
                                    onclick="imprimirTablaVacia('pending')"
                                    class="block w-full text-left px-4 py-2.5 text-sm font-medium text-slate-800 hover:bg-amber-50 hover:text-amber-800 transition-colors border-t border-slate-200">Imprimir pendientes
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <form id="formDespacho" class="bg-white overflow-hidden">
            ...
            <div id="tabsAjustesGlobales" class="px-6 py-3 border-b border-slate-200 bg-slate-50">
                <div class="flex items-center gap-2 flex-wrap text-xs">
                    <span class="text-slate-500 font-medium mr-1">Vista cantidades:</span>
                </div>
            </div>
            ...
            <div class="overflow-x-auto lg:overflow-visible">
                <table class="w-full text-sm min-w-[800px] lg:min-w-full border-collapse">
                    <thead class="bg-slate-50 border-b-2 border-slate-400 sticky top-0 z-50">
                        <tr>
                            <th class="px-2 lg:px-4 py-3 text-left font-medium text-slate-700 text-xs lg:text-sm border-r border-slate-400">Descripcion</th>
                            <th class="px-2 lg:px-4 py-3 text-center font-medium text-slate-700 w-16 text-xs lg:text-sm border-r border-slate-400">Genero</th>
                        ...
                        @if($prendas->count() > 0)
                            ...
                            @foreach($prendasAgrupadas as $prendaId => $filasGroup)
                                @php
                                    ...
                                    // Si la prenda tiene colores por talla, separar en sub-items por color Y genero
                                    $gruposPorColor = [];
                                            ...
                                            if (isset($variante['colores_detalle']) && is_array($variante['colores_detalle']) && !empty($variante['colores_detalle'])) {
                                                foreach ($variante['colores_detalle'] as $colorDetalle) {
                                                    ...
                                                    if (!empty($tallaVar) && $cantidadColor > 0) {
                                                        ...
                                                        // Nivel 2: por genero dentro del color
                                    ...
                                    // Ordenar tallas dentro de cada grupo genero
                                ...
                                @if($tieneColoresPorTalla)
                                    @foreach($gruposPorColor as $colorKey => $generosPorColor)
                                        @php
                                            ...
                                            // Total de filas para este color (descripcion rowspan)
                                            $totalRowsColor = 0;
                                        ...
                                        @foreach($generosPorColor as $generoKey => $tallasDelGenero)
                                            ...
                                            <tr class="border-b border-slate-400 hover:bg-slate-50"
                                                ...
                                                {{-- CELDA DE DESCRIPCION: Solo en la primera fila del color --}}
                                                        ...
                                                        <div class="font-semibold text-slate-900 mb-1 flex items-center gap-2 flex-wrap">
                                                            ...
                                                            @if($primeraFila->tiene_historial ?? false)
                                                                ...
                                                                <script type="text/javascript">
                                                                    console.log(' [BLADE] boton "Ver cambios" renderizado para PRENDA ID: {{ $primeraFila->id }}', {
                                                            ...
                                                            @else
                                                                <script type="text/javascript">
                                                                    console.log('INFO [BLADE] PRENDA SIN boton (no tiene_historial)', {
                                                                        prenda_id: {{ $primeraFila->id }},
                                                                        nombre: '{{ $primeraFila->objetoPrenda["nombre"] ?? $primeraFila->descripcion }}',
                                                ...
                                                {{-- CELDA DE GENERO: Solo en la primera fila de cada genero --}}
                                                @if($indexTalla === 0)
                                ...
                                @else
                                    @foreach($filasGroup as $indexFila => $fila)
                                        <tr class="border-b border-slate-400 hover:bg-slate-50" 
                                            ...
                                            {{-- CELDA DE DESCRIPCION: Solo en la primera fila del grupo --}}
                                            @if($indexFila === 0)
                                                <td class="px-2 lg:px-4 py-3 text-slate-900 text-xs col-descripcion" rowspan="{{ $rowSpan }}">
                                                    ...
                                                    @if($primeraFila->objetoPrenda && isset($primeraFila->objetoPrenda['variantes']) && is_array($primeraFila->objetoPrenda['variantes']) && count($primeraFila->objetoPrenda['variantes']) > 0)
                                                        @php
                                                            // Obtener la primera variante para mostrar las caracteristicas comunes
                                                            $primeraVariante = $primeraFila->objetoPrenda['variantes'][0];
                                                            $manga = $primeraVariante->manga ?? $primeraVariante['manga'] ?? null;
                                                            $manga_obs = $primeraVariante->manga_obs ?? $primeraVariante['manga_obs'] ?? '';
                                                            $broche = $primeraVariante->broche ?? $primeraVariante['broche'] ?? null;
                                                ...
                                                {{-- CELDA DE GENERO: Solo en la primera fila del grupo --}}
                                    ...
                                    <td class="px-2 lg:px-4 py-3 text-slate-900 text-xs col-descripcion">
                                        ...
                                        <div class="mt-2">
                                            @if($fila->tiene_historial ?? false)
                                                ...
                                                <script type="text/javascript">
                                                    console.log(' [BLADE] boton "Ver cambios" renderizado para EPP ID: {{ $fila->id }}', {
                                            ...
                                            @else
                                                <script type="text/javascript">
                                                    console.log(' [BLADE] EPP SIN boton (no tiene_historial)', {
                                                        epp_id: {{ $fila->id }},
                                                        nombre: '{{ $fila->objetoEpp["nombre"] ?? $fila->objetoEpp["nombre_completo"] ?? "N/A" }}',
            ...
            <div class="px-6 py-4 bg-slate-50 border-t-2 border-slate-400">
                <div class="text-sm text-slate-600">
                    <span class="font-medium">{{ $prendas->count() + $epps->count() }}</span> Items en total
                </div>
            </div>
        </form>
    </div>
...
<!-- Modal de Confirmacion para Deshacer Entregado -->
    ...
    try {
        ...
        if (data.success) {
            ...
            if (fechaCampo) {
                ...
                fechaCampo.readOnly = false; // Permitir edicion si se necesita
            }
            ...
            // Importante: habilitar el boton para permitir deshacer
            button.disabled = false;
            
            // Agregar efecto visual a la fila: color azul pastel
            fila.style.backgroundColor = '#DBEAFE'; // bg-blue-100 (azul pastel)
            fila.dataset.entregado = '1';
            inicializarAjustesCantidadesDespacho();
            
            console.log(' item marcado como entregado:', data);
        } else {
            // Error: restaurar boton
...
async function deshacerEntregado(button) {
    ...
    // Guardar referencia al boton y datos para usarlos despues
...
async function confirmarDeshacerEntregado() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Restaurar el boton a estado inicial
...
/**
 * Cargar estado inicial de entregas al cargar la pagina
 */
        ...
        if (data.success && data.entregas) {
            data.entregas.forEach(entrega => {
                ...
                if (fila) {
                    ...
                    if (button) {
                        // Marcar como entregado visualmente con opcion de deshacer
                        button.innerHTML = '✓ Entregado <span class="ml-1 text-xs">(↶)</span>';
                        button.classList.remove('bg-green-500', 'hover:bg-green-600');
                        button.classList.add('bg-orange-500', 'hover:bg-orange-600');
                        button.onclick = function() { deshacerEntregado(this); };
            ...
            console.log(' Estado de entregas cargado:', data.entregas.length, 'Items entregados');
            sincronizarEstadoEntregadoFilas();
            inicializarAjustesCantidadesDespacho();
        }
    } catch (error) {
...
function filaEstaEntregada(fila) {
    if (!fila) return false;
    if (fila.dataset.entregado === '1') return true;
    if (fila.dataset.entregado === '0') return false;

    const button = fila.querySelector('button[data-action="entrega"]');
    const onclickText = (button?.getAttribute('onclick') || '').toLowerCase();
    const tieneClaseEntregado = button?.classList?.contains('bg-orange-500');
    const apuntaADeshacer = onclickText.includes('deshacerentregado');

    // Respaldo: si hay fecha registrada, se considera entregado.
    const tipo = fila.dataset.tipo;
    const itemId = fila.dataset.id;
    const tallaId = fila.dataset.tallaId || '';
    const tallaColorId = fila.dataset.tallaColorId || '';
    const clave = tipo === 'epp'
        ? `epp-${itemId}`
        : `${itemId}${tallaId ? '-' + tallaId : ''}${tallaColorId ? '-' + tallaColorId : ''}`;
    const fechaCampo = document.getElementById(`fecha-entrega-${clave}`);
    const tieneFecha = Boolean((fechaCampo?.value || '').trim());

    return Boolean(tieneClaseEntregado || apuntaADeshacer || tieneFecha);
}

function sincronizarEstadoEntregadoFilas() {
    document.querySelectorAll('#tablaDespacho tr[data-tipo]').forEach((fila) => {
        fila.dataset.entregado = filaEstaEntregada(fila) ? '1' : '0';
    });
}

function toggleMenuImpresion(event) {
    event?.stopPropagation?.();
    const menu = document.getElementById('imprimirMenuOpciones');
    if (!menu) return;
    menu.classList.toggle('hidden');
}

function cerrarMenuImpresion() {
    const menu = document.getElementById('imprimirMenuOpciones');
    if (!menu) return;
    menu.classList.add('hidden');
}

window.despachoAjustesActivos = @json($ajustesActivos ?? []);
window.despachoAjustesHistorial = @json($ajustesHistorial ?? []);
window.despachoVistaSeleccionada = 'original';

function construirClaveAjusteDespacho(fila) {
    const tipoItem = String(fila.dataset.tipo || '').trim().toLowerCase();
    const itemId = parseInt(fila.dataset.id || '0', 10) || 0;
    const tallaId = parseInt(fila.dataset.tallaId || '0', 10) || 0;
    const tallaColorId = parseInt(fila.dataset.tallaColorId || '0', 10) || 0;
    const genero = String(fila.dataset.genero || '').trim().toUpperCase();
    return `${tipoItem}|${itemId}|${tallaId}|${tallaColorId}|${genero}`;
}

function construirTabsGlobalesAjustes() {
    const wrap = document.getElementById('tabsAjustesGlobales');
    if (!wrap) return;
    ...
    const params = new URLSearchParams(window.location.search);
    const tabDesdeUrl = (params.get('tab') || '').toLowerCase();
    if (tabDesdeUrl === 'pendientes' || tabDesdeUrl === 'original') {
        window.despachoVistaSeleccionada = tabDesdeUrl;
    }

    const tabs = [
        { label: 'Original', vista: 'original' },
        { label: 'Pendientes', vista: 'pendientes' },
    ];

    tabs.forEach((tab, idx) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.dataset.vista = tab.vista;
        const isActive = window.despachoVistaSeleccionada === tab.vista
            || (window.despachoVistaSeleccionada !== 'pendientes' && idx === 0 && tab.vista === 'original');
        btn.className = 'px-5 py-2 rounded-lg border text-sm font-semibold transition-colors ' + (isActive
            ? 'bg-slate-900 text-white border-slate-900'
            : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-100');
        btn.textContent = tab.label;
        btn.addEventListener('click', () => {
            window.despachoVistaSeleccionada = tab.vista;
            inner.querySelectorAll('button[data-vista]').forEach((b) => {
                b.className = 'px-5 py-2 rounded-lg border text-sm font-semibold transition-colors bg-white text-slate-700 border-slate-300 hover:bg-slate-100';
            });
            btn.className = 'px-5 py-2 rounded-lg border text-sm font-semibold transition-colors bg-slate-900 text-white border-slate-900';

            const url = new URL(window.location.href);
            url.searchParams.set('tab', tab.vista);
...
function cantidadPendiente(historial, original) {
    if (!Array.isArray(historial) || historial.length === 0) {
        return original;
    }

    const ultimo = [...historial]
        .sort((a, b) => (parseInt(b.revision || 0, 10) || 0) - (parseInt(a.revision || 0, 10) || 0))[0];
    return parseInt(ultimo?.cantidad_ajustada || original, 10) || original;
}

function inicializarAjustesCantidadesDespacho() {
    const historialMap = window.despachoAjustesHistorial || {};
    const vista = window.despachoVistaSeleccionada || 'original';
    document.querySelectorAll('#tablaDespacho tr[data-tipo]').forEach((fila) => {
        const cantidadCell = fila.querySelector('td.col-cantidad');
        if (!cantidadCell) return;
        ...
        fila.dataset.rowHash = rowKey;
        fila.dataset.cantidadOriginal = String(baseVigente);
        fila.dataset.cantidadAjustada = String(cantidadVista);
        ...
        const input = cantidadCell.querySelector('.cantidad-ajustada-input');
        if (!input) return;
        ...
        input.addEventListener('keydown', async (event) => {
            ...
            try {
                input.disabled = true;
                await guardarAjusteFilaDespacho(fila);
                fila.dataset.cantidadOriginal = String(fila.dataset.cantidadAjustada || input.value);
                input.disabled = false;

                const box = cantidadCell.querySelector('.flex.flex-col');
                if (box) {
                    box.classList.add('bg-emerald-100', 'ring-1', 'ring-emerald-400', 'rounded');
                    setTimeout(() => {
                        box.classList.remove('bg-emerald-100', 'ring-1', 'ring-emerald-400', 'rounded');
                    }, 1800);
                }
            } catch (error) {
                input.disabled = false;
...
async function guardarAjusteFilaDespacho(fila) {
    const input = fila.querySelector('.cantidad-ajustada-input');
    if (!input) {
        throw new Error('La fila no es editable.');
    }
    if (filaEstaEntregada(fila)) {
        throw new Error('No se puede ajustar una fila entregada.');
    }
...
async function guardarAjustesCantidadesDespacho() {
    ...
    const cambios = filas.filter((fila) => {
        ...
        const original = parseInt(fila.dataset.cantidadOriginal || '0', 10) || 0;
        const ajustada = parseInt(fila.dataset.cantidadAjustada || '0', 10) || 0;
        return ajustada !== original;
    });

    if (cambios.length === 0) {
        alert('No hay cambios en cantidades para guardar.');
        return;
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    construirTabsGlobalesAjustes();
    inicializarAjustesCantidadesDespacho();

    document.addEventListener('click', function (event) {
        const wrapper = document.getElementById('imprimirMenuWrapper');
        if (!wrapper) return;
...
async function abrirModalFactura(pedidoId) {
    ...
    try {
        ...
        try {
            ...
            contenido.innerHTML = '<div class="text-center text-red-600 py-6"> Error: respuesta invalida del servidor (no JSON)</div>';
            return;
        }

        if (data) {
            // DEBUG: Ver que datos estamos recibiendo
...
/**
 * Generar HTML de la factura - VERSION BODEGA
 */
function generarHTMLFactura(datos) {
    ...
    if (datos.prendas && datos.prendas[0]) {
        ...
        console.log(' [DESPACHO-FACTURA] Descripcion:', datos.prendas[0].descripcion);
                    ...
                    ${imagenes.length > 0 && primeraImagen ? `
                        <div style="margin-top: 6px; padding-top: 6px; border-top: 1px dashed #e5e7eb;">
                            ...
                            <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                                <div style="position: relative; cursor: pointer;" onclick="window._abrirGaleriaImagenes(${JSON.stringify(imagenes).replace(/\"/g, '&quot;')}, 'Imagenes de ${proc.tipo || proc.nombre || 'Proceso'}')">
    ...
    const prendasHTML = datos.prendas.map((prenda, idx) => {
        // Usar TALLAS primero que es donde estan los datos correctos
        let variantesHTML = '';
        if (prenda.tallas && typeof prenda.tallas === 'object' && Object.keys(prenda.tallas).length > 0) {
            ...
            // Convertir objeto de generos a array de tallas con genero y colores
            let todasLasTallas = [];
                ...
                if (typeof prenda.tallas[genero] === 'object') {
                    Object.entries(prenda.tallas[genero]).forEach(([talla, cantidad]) => {
                        ...
                        todasLasTallas.push({ 
                            ...
                            colores: [] // Se llenara con los colores de prenda_pedido_talla_colores
                        });
                    });
                }
            });
            ...
            if (todasLasTallas.length > 0) {
                // Verificar si hay colores para decidir que tabla mostrar
                const tieneColores = todasLasTallas.some(t => t.colores && t.colores.length > 0);
                
                // Verificar si todas las tallas son "—" (UNISEX sin talla especifica)
                        ...
                        <table style="width: 100%; font-size: 13px; border-collapse: collapse;">
                            <thead>
                                <tr style="background: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                                    <th style="padding: 6px 8px; text-align: left; font-weight: 600; color: #374151;">Genero</th>
        ...
        <div style="margin: 12px 0; padding: 0; background: #ffffff; border-radius: 6px; border: 1px solid #e0e7ff; overflow: hidden;">
            ...
            <div style="padding: 12px; space-y: 8px;">
                ${datos.epps.map(epp => {
                    ...
                    console.log(' [DESPACHO-FACTURA] EPP con imagenes:', {
                    ...
                    // Generar HTML para imagenes si existen
                    const imagenesHTML = (epp.imagenes && Array.isArray(epp.imagenes) && epp.imagenes.length > 0) ? `
                        <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #e5e7eb;">
                            <div style="color: #6b7280; font-size: 11px; text-transform: uppercase; margin-bottom: 4px; font-weight: 600;"> Imagenes (${epp.imagenes.length})</div>
    ...
    const totalHTML = `
        <div style="margin: 12px 0; padding: 12px; background: #f3f4f6; border-radius: 6px; border: 2px solid #d1d5db; text-align: right;">
            <div style="font-size: 12px; margin-bottom: 8px;">
                <strong>Total items:</strong> ${datos.total_items || 0}
            </div>
            ${datos.valor_total ? `
                <div style="font-size: 12px; margin-bottom: 8px;">
        ...
        <div>
            ...
            <div style="background: #1e3a8a; color: white; padding: 16px; border-radius: 6px; margin-bottom: 12px; text-align: center;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; font-size: 12px; margin-top: 12px;">
                    <div>
                        <div style="font-size: 10px; opacity: 0.8;">Numero</div>
 ...
 */
function imprimirTablaVacia(modo = 'all') {
    cerrarMenuImpresion();
    sincronizarEstadoEntregadoFilas();
    ...
    // Construir tabla HTML con 11 columnas para impresion
    let tablaHTML = `
        <table style="width: 100%; border-collapse: collapse; border: 2px solid #000;">
            <thead style="background: #f1f5f9; border-bottom: 2px solid #000;">
                <tr>
                    <th style="padding: 8px 4px; text-align: left; font-weight: 600; font-size: 11px; border: 1px solid #000;">Descripcion</th>
                    <th style="padding: 8px 4px; text-align: center; font-weight: 600; font-size: 11px; border: 1px solid #000; width: 60px;">Genero</th>
    ...
    const todasLasFilas = Array.from(document.querySelectorAll('#tablaDespacho tr[data-tipo]'));
    const filas = modo === 'pending'
        ? todasLasFilas.filter((fila) => !filaEstaEntregada(fila))
        : todasLasFilas;
    let ultimoTipo = '';
    let ultimoGrupoDescripcion = '';
    let ultimoId = null;
    let grupoFilas = [];
    const limpiarCellHtml = (cell) => {
        if (!cell) return '';
    ...
    const obtenerDescripcionDeFila = (fila) => {
        const descDirecta = fila.querySelector('td.col-descripcion');
        if (descDirecta) return limpiarCellHtml(descDirecta);

        const tipo = fila.dataset.tipo;
        const id = fila.dataset.id;
        let cursor = fila.previousElementSibling;
        while (cursor && cursor.matches('tr[data-tipo]')) {
            if (cursor.dataset.tipo !== tipo || cursor.dataset.id !== id) break;
            const desc = cursor.querySelector('td.col-descripcion');
            if (desc) return limpiarCellHtml(desc);
    ...
    const obtenerCantidadParaImprimir = (fila, modoActual) => {
        const original = parseInt(fila.dataset.cantidadOriginalInicial || '0', 10) || 0;
        ...
        if (modoActual === 'pending') {
            ...
            const historial = Array.isArray((window.despachoAjustesHistorial || {})[key])
                ...
                : [];
            return String(cantidadPendiente(historial, original));
        }
    ...
    const registros = filas.map((fila) => {
        ...
        const descripcionHtml = descCell ? limpiarCellHtml(descCell) : obtenerDescripcionDeFila(fila);
        const descripcionTexto = descripcionHtml.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();

        return {
            tipo,
            id,
            claveGrupo: `${tipo}-${id}-${descripcionTexto}`,
            descripcionHtml,
            genero: limpiarCell(generoCell) || (fila.dataset.genero || '').trim() || '—',
            talla: limpiarCell(tallaCell) || '—',
            cantidad: obtenerCantidadParaImprimir(fila, modo),
        };
    });
    ...
    for (let i = 0; i < registros.length; i++) {
        ...
        if (row.tipo !== ultimoTipo) {
            const nombreSeccion = row.tipo === 'prenda' ? 'Prendas' : 'EPP';
            tablaHTML += `
            ...
            `;
            ultimoTipo = row.tipo;
            ultimoGrupoDescripcion = '';
        }

        const grupoInicia = i === 0 || registros[i - 1].claveGrupo !== row.claveGrupo;
        let rowspan = 1;
        if (grupoInicia) {
            for (let j = i + 1; j < registros.length; j++) {
                if (registros[j].claveGrupo !== row.claveGrupo) break;
                rowspan++;
            }
        }
        ...
        tablaHTML += `
            <tr style="border-bottom: 1px solid #000;">
                ${grupoInicia ? `<td style="padding: 8px 4px; font-size: 10px; border: 1px solid #000;" rowspan="${rowspan}">${row.descripcionHtml}</td>` : ''}
                <td style="padding: 8px 4px; text-align: center; font-size: 10px; border: 1px solid #000;">${row.genero}</td>
                <td style="padding: 8px 4px; text-align: center; font-size: 10px; border: 1px solid #000;">${row.talla}</td>
                <td style="padding: 8px 4px; text-align: center; font-weight: 600; font-size: 10px; border: 1px solid #000;">${row.cantidad}</td>
...
if (filas.length === 0) {
        tablaHTML += `
            <tr>
                <td colspan="11" style="padding: 12px; text-align: center; font-size: 11px; border: 1px solid #000;">
                    No hay Items pendientes para imprimir.
                </td>
            </tr>
        `;
    }
        ...
        <body>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; padding: 20px; border: 3px solid #000; border-radius: 10px; background: #f9fafb;">
                ...
                <div style="text-align: right;">
                    <p style="margin: 0; font-size: 11px; color: #666;"><strong>Fecha de impresion:</strong></p>
