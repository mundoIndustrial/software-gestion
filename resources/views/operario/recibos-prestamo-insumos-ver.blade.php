...
<div id="firma-modal-backdrop" class="firma-modal-backdrop">
    <div class="firma-modal">
        ...
        <div class="firma-modal-actions">
            <button type="button" id="btn-limpiar-firma" class="btn-firma" style="background:#fff;color:#111827;">Limpiar</button>
            <button type="button" id="btn-cancelar-firma" class="btn-firma" style="background:#fff;color:#111827;">Cancelar</button>
            <button type="button" id="btn-guardar-firma" class="btn-firma">Guardar firma</button>
        </div>
    </div>
</div>
...
(() => {
    ...
    saveBtn?.addEventListener('click', async () => {
        if (!hasStroke) {
            alert('Debes dibujar una firma primero.');
            return;
        }
        ...
        try {
            const response = await fetch(`{{ route('operario.recibos-prestamo.insumos.firma', ['id' => $recibo->id, 'firmante' => '__FIRMANTE__']) }}`.replace('__FIRMANTE__', firmanteActual), {
                method: 'POST',
                headers: {
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || 'No fue posible guardar la firma.');
            }
    ...
    if (firmanteQuery === 'costurero' || firmanteQuery === 'mensajero') {
        openModal(firmanteQuery);
    }
})();
</script>
