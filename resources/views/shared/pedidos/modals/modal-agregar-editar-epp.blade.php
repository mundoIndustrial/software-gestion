...
function abrirModalAgregarEPP() {
    ...
    if (window.DragDropManager && typeof window.DragDropManager.registrarSubModal === 'function') {
        window.DragDropManager.registrarSubModal('modalAgregarEPP', function(file) {
            ...
            // Mostrar sección de edición para que el usuario pueda crear un EPP
            const seccionFotos = document.getElementById('seccionFotosEPP');
            if (seccionFotos) {
                seccionFotos.style.display = 'block';
                console.log('[Paste Handler EPP] Sección de edición mostrada');
            }
...
function prepararDatosParaEnvio(itemsPedido) {
    ...
    itemsPedido.forEach((item, index) => {
        ...
        // Caso legacy: item es un EPP individual con imágenes
            ...
            // Agregar outros items sin modificar, asegurando modo_imagenes sólo en EPPs individuales
            if (item.tipo === 'epp') {
...
async function finalizarAgregarEPP() {
    ...
    const promesasEPP = eppAgregadosList.map(async (epp) => {
        ...
        const eppData = {
            ...
            modo_imagenes: (imagenesParaGuardar && imagenesParaGuardar.length > 0) ? 'upload' : 'reuse' // Indicar modo de imágenes
        };
