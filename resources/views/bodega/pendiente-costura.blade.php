...
@section('title', 'Pendientes de Costura - Bodega')
@section('page-title', 'Pendientes de Costura')
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="mb-6">
            <form method="GET" class="flex gap-3">
                ...
                @if($search ?? false)
                    <a
                        href="{{ route('gestion-bodega.pendientes-costura') }}"
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200">
                            ...
                            @foreach($pedidosPorPagina as $pedido)
                                ...
                                <tr class="hover:bg-slate-50 transition-colors {{ $rowBg }}">
                                    <td class="px-6 py-4 text-center">
                                        <a href="{{ route('gestion-bodega.pendiente-costura-show', $pedido['id']) }}" class="inline-block px-3 py-1 bg-slate-900 hover:bg-slate-800 text-white text-xs font-medium rounded transition-colors">Ver</a>
                                    </td>
                                    @if($puedeMarcarVisto)
                                        <td class="px-6 py-4 text-center">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-slate-500 font-medium mt-4">No hay pedidos pendientes de costura</p>
                    <p class="text-slate-400 text-sm mt-2">No se encontraron registros con estado "Pendiente" en el área "Costura"</p>
...
function cargarDatosFiltro() {
    ...
    const url = `/gestion-bodega/filtro-datos/${tipoFiltroActual}`;
...
function toggleSeleccionarTodo() {
    ...
    btn.textContent = todosSeleccionados ? 'Seleccionar todo' : 'Deseleccionar todo';
    actualizarContadorSeleccionados();
}
...
function verificarFiltrosActivos() {
    ...
    const filtrosActivos = ['numero_pedido', 'cliente', 'asesor', 'estado', 'fecha_creacion', 'retrasados'];
