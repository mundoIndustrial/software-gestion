                    ...
                    <tbody id="pedidosTableBody" class="divide-y divide-slate-200">
                        @forelse($pedidosAgrupados as $numeroPedido => $items)
                            ...
                            @foreach($items as $item)
                                @php
                                    // Determinar si se permite editar basado en el rol
                                    $rolActual = auth()->user()->getCurrentRole()?->name ?? '';
                                    $puedeEditar = in_array($rolActual, ['Costura-Bodega', 'Epp-Bodega']);
                                @endphp
                                <tr class="pedido-row"
                                    data-numero-pedido="{{ $numeroPedido }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const estadoSelects = document.querySelectorAll('.estado-select-readonly');
...
/**
 * Abrir modal para editar una nota
 */
