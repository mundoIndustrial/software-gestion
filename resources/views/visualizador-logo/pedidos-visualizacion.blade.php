...
@section('title', 'Visualización de Pedidos')
...
<div style="padding: 1rem 1rem 2rem 1rem; background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); min-height: calc(100vh - 60px); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
    ...
    <div style="display: flex; justify-content: center;">
        <div style="width: 100%; max-width: 1400px;">
            <!-- Container -->
            <div style="background: white; border-radius: 12px; overflow: visible; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.07), 0 1px 3px rgba(0, 0, 0, 0.06); border: 1px solid #e2e8f0; position: relative;">
                <!-- Header -->
                <div style="
                    ...
                    padding: 1rem 1.5rem;
                    display: grid;
                    grid-template-columns: 80px 100px 200px 150px 120px;
                    gap: 1rem;
                    font-weight: 700;
                    font-size: 0.9rem;
                ...
                <!-- Filas -->
                <div id="pedidos-body" style="overflow: visible;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function cargarPedidos(searchTerm = '') {
        ...
        const newUrl = `{{ route("visualizador-logo.pedidos-visualizacion") }}?${params.toString()}`;
        ...
        fetch(`{{ route("visualizador-logo.pedidos-visualizacion.data") }}?${params.toString()}`)
                ...
                " onmouseover="this.style.background='#f8fafc'; this.style.boxShadow='inset 0 0 0 1px #e2e8f0'" onmouseout="this.style.background='white'; this.style.boxShadow='none'">
                    ...
                    <div style="display: flex; justify-content: center; gap: 0.5rem; position: relative;">
                        ...
                        <div id="menu-ver-${pedido.id}" class="dropdown-menu" style="display: none; position: absolute; top: 100%; right: 0; background: white; border: 1px solid #d1d5db; border-radius: 12px; box-shadow: rgba(15, 23, 42, 0.22) 0px 14px 30px; z-index: 1000; min-width: 220px; margin-top: 0.5rem; overflow: hidden;">
                            ...
                            <button type="button" onclick="openPedidoRecibos(${pedido.id})" style="width: 100%; border: none; background: white; padding: 12px 14px; text-align: left; cursor: pointer; color: #111827; font-size: 15px; font-weight: 600; line-height: 1.2; transition: background-color 0.15s;" onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background='white'">
                                <span style="display: inline-flex; align-items: center; gap: 10px;">
                                    <i class="fas fa-receipt" style="width: 16px; text-align: center; color: #374151;"></i>
                    ...
                    <div style="color: #334155; font-size: 0.95rem; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${cliente}</div>
                    <div style="color: #64748b; font-size: 0.95rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${asesora}</div>
                    <div style="color: #64748b; font-size: 0.95rem;">${formatearFecha(pedido.fecha_pedido || pedido.created_at)}</div>
                </div>
            `;
        }).join('');
